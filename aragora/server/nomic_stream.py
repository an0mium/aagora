...
def _create_cycle_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_cycle_start(cycle_num: int, max_cycles: int, started_at: str) -> None:
        _emit(emitter, StreamEventType.CYCLE_START, {
            "cycle": cycle_num,
    ...
    def on_cycle_end(cycle_num: int, success: bool, duration_seconds: float, outcome: str) -> None:
        _emit(emitter, StreamEventType.CYCLE_END, {
            "cycle": cycle_num,
            "success": success,
...
def _create_phase_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_phase_start(phase: str, cycle: int, details: Optional[dict] = None) -> None:
        _emit(emitter, StreamEventType.PHASE_START, {
            "phase": phase,
            "cycle": cycle,
            **(details or {}),
        })
    ...
    def on_phase_end(
        phase: str, cycle: int, success: bool, duration_seconds: float,
        result: Optional[dict] = None
    ) -> None:
        _emit(emitter, StreamEventType.PHASE_END, {
            "phase": phase,
            "cycle": cycle,
            "success": success,
            "duration_seconds": duration_seconds,
...
def _create_task_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_task_start(
        task_id: str, description: str, complexity: str, model: str,
        total_tasks: int, completed_tasks: int
    ) -> None:
        _emit(emitter, StreamEventType.TASK_START, {
            "task_id": task_id,
    ...
    def on_task_complete(
        task_id: str, success: bool, duration_seconds: float,
        diff_preview: str = "", error: Optional[str] = None
    ) -> None:
        _emit(emitter, StreamEventType.TASK_COMPLETE, {
            "task_id": task_id,
            "success": success,
            "duration_seconds": duration_seconds,
    ...
    def on_task_retry(task_id: str, attempt: int, reason: str, timeout: int) -> None:
        _emit(emitter, StreamEventType.TASK_RETRY, {
            "task_id": task_id,
            "attempt": attempt,
            "reason": reason,
            "timeout": timeout,
...
def _create_verification_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_verification_result(check_name: str, passed: bool, message: str = "") -> None:
        _emit(emitter, StreamEventType.VERIFICATION_RESULT, {
            "check": check_name,
            "passed": passed,
            "message": message,
        })
...
def _create_backup_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_commit(commit_hash: str, message: str, files_changed: int) -> None:
        _emit(emitter, StreamEventType.COMMIT, {
    ...
    def on_backup_created(backup_name: str, files_count: int, reason: str) -> None:
        _emit(emitter, StreamEventType.BACKUP_CREATED, {
    ...
    def on_backup_restored(backup_name: str, files_count: int, reason: str) -> None:
        _emit(emitter, StreamEventType.BACKUP_RESTORED, {
            "backup_name": backup_name,
            "files_count": files_count,
            "reason": reason,
        })
...
def _create_log_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_error(phase: str, message: str, recoverable: bool = True) -> None:
        _emit(emitter, StreamEventType.ERROR, {
            "phase": phase,
            "message": message,
    ...
    def on_log_message(
        ...
        phase: Optional[str] = None, agent: Optional[str] = None
    ) -> None:
        _emit(emitter, StreamEventType.LOG_MESSAGE, {
            "message": message,
            "level": level,
            "phase": phase,
        }, agent=agent or "")
    ...
    def on_match_recorded(
        debate_id: str, participants: list[str], elo_changes: dict[str, float],
        domain: Optional[str] = None, winner: Optional[str] = None,
        loop_id: Optional[str] = None
    ) -> None:
        _emit(emitter, StreamEventType.MATCH_RECORDED, {
            "debate_id": debate_id,
...
def _create_probe_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_probe_start(
        ...
        _emit(emitter, StreamEventType.PROBE_START, {
            "probe_id": probe_id,
    ...
    def on_probe_result(
        probe_id: str, probe_type: str, passed: bool,
        severity: Optional[str] = None, description: str = "", response_time_ms: float = 0
    ) -> None:
        _emit(emitter, StreamEventType.PROBE_RESULT, {
            "probe_id": probe_id,
    ...
    def on_probe_complete(
        report_id: str, target_agent: str, probes_run: int,
        vulnerabilities_found: int, vulnerability_rate: float,
        elo_penalty: float, by_severity: Optional[dict] = None
    ) -> None:
        _emit(emitter, StreamEventType.PROBE_COMPLETE, {
            "report_id": report_id,
            "target_agent": target_agent,
...
def _create_audit_hooks(emitter: SyncEventEmitter) -> dict[str, Callable]:
    ...
    def on_audit_start(
        ...
        _emit(emitter, StreamEventType.AUDIT_START, {
            "audit_id": audit_id,
            "task": task,
            "agents": agents,
            "config": config or {},
    ...
    def on_audit_round(
        audit_id: str, round_num: int, round_name: str,
        cognitive_role: str, messages: list[dict], duration_ms: float = 0
    ) -> None:
        _emit(emitter, StreamEventType.AUDIT_ROUND, {
            "audit_id": audit_id,
            "round": round_num,
            "name": round_name,
    ...
    def on_audit_finding(
        audit_id: str, category: str, summary: str, details: str,
        agents_agree: list[str], agents_disagree: list[str],
        confidence: float, severity: float = 0.0
    ) -> None:
        _emit(emitter, StreamEventType.AUDIT_FINDING, {
            "audit_id": audit_id,
            "category": category,
            "summary": summary,
            "details": details,
    ...
    def on_audit_cross_exam(
        ...
        _emit(emitter, StreamEventType.AUDIT_CROSS_EXAM, {
            "audit_id": audit_id,
    ...
    def on_audit_verdict(
        audit_id: str, task: str, recommendation: str, confidence: float,
        unanimous_issues: list[str], split_opinions: list[str],
        risk_areas: list[str], rounds_completed: int, total_duration_ms: float,
        agents: list[str], elo_adjustments: Optional[dict] = None
    ) -> None:
        _emit(emitter, StreamEventType.AUDIT_VERDICT, {
            "audit_id": audit_id,
            "task": task,
